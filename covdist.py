...
def thetayc(vec1, vec2):
    ...
    if np.sum(vec1) == 0 or np.sum(vec2) == 0:
        ...
        dist = np.sum(np.square(vec1), axis=0) + \
            np.sum(np.square(vec2), axis=0)
        if dist == dotsum:
            return 0
        else:
...
def CalculateDist(freq_table1, freq_table2):
    ...
    for key in key_inter:
        if key not in freq_table1.filtered_pos + freq_table2.filtered_pos:
            sumval += thetayc(freq_table1[key].freq_vec,
                              freq_table2[key].freq_vec)
    for key in (set(freq_table1) - key_inter):
        if key not in freq_table1.filtered_pos + freq_table2.filtered_pos:
            sumval += thetayc(freq_table1[key].freq_vec,
                              RefTableFreq[key].freq_vec)
    for key in (set(freq_table2) - key_inter):
        if key not in freq_table1.filtered_pos + freq_table2.filtered_pos:
            sumval += thetayc(freq_table2[key].freq_vec,
                              RefTableFreq[key].freq_vec)
    return sumval
...
def depth2Dict(depth_file):
    ...
    df = pd.read_csv(depth_file, sep="\t", header=0, names=[
                     "Ref_id", "Pos", "Ref", "Depth"])
...
def vcf2FreqTable(prefix, refpath, cov_depth_cutoff, required_coverage, mode="normal"):
    ...
    if not vcf_abf:
        raise FileNotFoundError(
            "Vcf file: {} doesn't exist! Terminate program.".format(prefix+'.vcf'))
    ...
    if not depth_abf and mode == "normal":
        logger.warning(
            'Depth file: {} is not found. Default to set all position depths are valid.'.format(prefix+'.depth'))
    ...
    for contig_id in sorted(list(contigs.keys())):
        ...
        if depth_abf:
            ...
            coverage = calculate_coverage(
                pos_depth_dict, contig, cov_depth_cutoff)
            if coverage < required_coverage:
                logger.warning('{0} coverage ({1}%) is less than {2}% genome coverage and it will be removed from analysis!'.format(
                    vcf_abf, coverage*100, required_coverage*100))
                return None
        ...
        for record in vcf_reader:
            ...
            if record.is_snp and record.CHROM == contig.id:
                    ...
                    if str(alt) in 'ACGT':
                        if 'AF' in record.INFO:
                            if type(record.INFO['AF']) != float:
                                freq_vector['ACGT'.index(
                                    str(alt))] = record.INFO['AF'][index]
                            else:
                                freq_vector['ACGT'.index(
                                    str(alt))] = record.INFO['AF']
                        else:
                ...
                if 1 - sum(freq_vector) > 1e-5:
                    freq_vector['ACGT'.index(
                        str(record.REF))] = 1 - sum(freq_vector)
...
def vcf2FreqTabDict(prefixlist, refpath, cov_depth_cutoff, required_coverage, cpus, mode="normal"):
    ...
    with Pool(cpus) as p:
        try:
            list_dt = p.map(partial(vcf2FreqTable, refpath=refpath, cov_depth_cutoff=cov_depth_cutoff,
                                    required_coverage=required_coverage, mode=mode), prefixlist)
...
@cli.command()
@click.option('-p', '--prefix_list', required=True, type=click.Path(exists=True), help='A file that lists the prefix path for vcf and corresponding depth files.')
...
def dist(prefix_list, refpath, cov_depth_cutoff, required_coverage, cpus, voc_dir, outpath):
    ...
    freq_tab_dict = vcf2FreqTabDict(
        prefixlist, refpath, cov_depth_cutoff, required_coverage, cpus, mode="normal")
    global RefTableFreq
    ...
    if not voc_dir is None:
        ...
        voc_freq_tab_dict = vcf2FreqTabDict(
            voc_prefixlist, refpath, cov_depth_cutoff, required_coverage, cpus, mode="voc")
...
###########################
my_symbols = ['diamond', 'x', 'circle', 'square',
              'cross', 'diamond-open', 'circle-open', 'square-open']
...
def validate_date_format(date_text):
    try:
        ...
        raise ValueError(
            'Incorrect data format for "{}", should be YYYY-MM-DD!'.format(date_text))
...
def check_meta_column(column, target_columns):
    if column != "default" and column not in target_columns:
        logger.error(
            'Column "{}" is not in meta data. Please check!'.format(column))
        sys.exit()
...
def default_plot(df, analysis, intersect_len, outpath, axis_names, hovertmp, voc=False, mode="3d"):
    ...
    date_to_val = df['collection_date'].map(
        pd.Series(data=np.arange(len(unique_date)), index=unique_date).to_dict())
    ...
    ticktext = [df['collection_date'][id].strftime(
...
def plot(distance_file, meta, column, analysis, voc_meta, outdir):
    ...
    merged_metadata['collection_site'] = merged_metadata['collection_site'].astype(
        'string')
    merged_metadata['collection_date'] = pd.to_datetime(
        merged_metadata['collection_date']).dt.date

    # check if distance samples are in metadata
    diff_samples = set(sample_names).difference(
        set(list(merged_metadata['sample'])))
    diff_len = len(diff_samples)
    if diff_len == 1:
       logger.warning('One sample ({}) is not found in metadata.'.format(
           list(diff_samples)[0]))
    elif 1 < diff_len <= 10:
        logger.warning('{0} samples ({1}) are not found in metadata.'.format(
            diff_len, ", ".join(list(diff_samples))))
    elif diff_len > 10:
        logger.warning('{0} samples ({1}) ... are not found in metadata.'.format(
            diff_len, ", ".join(list(diff_samples)[:10])))

    intersect_samples = list(set(sample_names).intersection(
        set(list(merged_metadata['sample']))))
    ...
    if analysis == "PCoA":
        ...
        axis_names = ["PC" + str(index+1) + " (" + '{:.2f}'.format(
            prop*100) + "%)" for index, prop in enumerate(ordination_result.proportion_explained[:3])]
    ...
    # this step is important for mapping and sorting collection_date column
    ...
    df.to_csv("{0}/{1}_ordination_result.txt".format(outdir,
                                                     analysis), index=False, sep="\t")
